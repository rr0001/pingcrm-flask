<template>
  <div>
    <div class="flex justify-start max-w-3xl mb-8">
      ...
      <img v-if="user.photo_path" class="block w-8 h-8 ml-4 rounded-full" :src="user.photo_path" />
    </div>
    ...
    <div class="max-w-3xl overflow-hidden bg-white rounded-md shadow">
      <form @submit.prevent="update">
        <div class="flex flex-wrap p-8 -mb-8 -mr-6">
          ...
          <file-input v-model="form.photo" :error="form.errors.photo" class="w-full pb-8 pr-6 lg:w-1/2" type="file" accept="image/*" label="Photo" @input="inputImage"/>
        </div>
