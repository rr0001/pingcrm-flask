  ...
  <div>
    ...
    >
      <div class="flex items-center">
        <svg
          class="w-4 h-4 ml-4 mr-2 shrink-0 fill-white"
        ...
        <div
          ...
          v-for="(success, index) in $page.props.flash.success"
          :key="index"
        >
          {{ success }}
        </div>
    ...
    <div v-if="$page.props.flash.error && show" class="flex items-center justify-between max-w-3xl mb-8 bg-red-400 rounded">
      <div class="flex items-center">
        <svg
          class="w-4 h-4 ml-4 mr-2 shrink-0 fill-white"
          xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          class="py-4 text-sm font-medium text-white"
          v-for="(error, index) in $page.props.flash.error"
          :key="index"
        >
          {{ error }}
        </div>
